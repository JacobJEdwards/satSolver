...
analyseConflict conflict = do
  SolverState {trail, decisionLevel, implicationGraph} <- get

  traceM $ "Analyse conflict: " ++ show conflict
  traceM $ "Trail: " ++ show trail
  traceM $ "Decision level: " ++ show decisionLevel
